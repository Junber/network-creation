...
def main():
	for choices in itertools.product([True, False], repeat=2):
		...
		for v in var_names(n):
			vars[v] = model.NewIntVar(1, var_upper_bound, f'd{v}')
		...
		for path in bad_paths:
			...
			for i in range(len(path) - 2):
				greedy_var_counter += 1 
				var = model.NewIntVar(1, var_upper_bound, f'greedy{greedy_var_counter}')
				greedyVars.append(var)
				model.AddMinEquality(var, [var_upper_bound, var_upper_bound - 1  + vars[name(path[i], end)] - vars[name(path[i+1], end)]])
				#print(var, " == MIN(", var_upper_bound, ",", var_upper_bound - 1  + vars[name(path[i], end)] - vars[name(path[i+1], end)], ")")
				greedy += var

			#greedy == var_upper_bound*(len(path) - 2)   <=>  path is greedy    (otherwise is it smaller)
			#Stretch above t or path not greedy
			model.Add(length + (len(path) - 2) * var_upper_bound * var_upper_bound > t + greedy * var_upper_bound)
			#print(length + (len(path) - 2) * var_upper_bound * var_upper_bound > t + greedy * var_upper_bound)
		...
		for v in var_names(n):
			objective += vars[v]
		model.Minimize(1000*t + objective)
