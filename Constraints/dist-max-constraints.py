...
def main():
	for choices in itertools.product([True, False], repeat=2):
		...
		#Candidate a
		model.Add(alpha_mult*(yz + zb - yb) > alpha)
		model.Add(alpha_mult*(ya + ab - yb) > alpha)
		...
		#Candidate c
		model.Add(yz + yb < zb + bc)

		#Candidate d
		model.Add(alpha_mult*(ya+ab - (yb+bc)) < alpha) # yab < ybc + alpha
		model.Add(alpha_mult*(yz+zc - (yb+bc)) < alpha) # yzc < ybc + alpha
		...
		model.Add(alpha_mult*(ab + bc + bc - (za + yz)) > alpha) #a will build to b

		model.Add(alpha_mult*(yb + ab - (yb + bc)) > alpha) #y will build to a
		model.Add(alpha_mult*(yb + bc - yc) < alpha) #y will not build both b and c
		#y will not build c
		model.Add(yc > yb)
		model.Add(ya + ab > yb + bc) #yab > ybc
		...
		model.Add(alpha_mult*(zb+bc) < alpha_mult*zc + alpha)

		#z will not build a
		model.Add(alpha_mult*(yz+ya) < alpha_mult*za + alpha)
